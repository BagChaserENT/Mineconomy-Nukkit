...
public class AddMoneyCommand extends Command {
    ...
    public AddMoneyCommand(String name){
        super(name, "Add money to someone");
        this.setAliases(new String[]{"addmoney", "add"});
    ...
    public boolean execute(CommandSender sender, String label, String[] args){
        ...
        if(money < 0){
            sender.sendMessage("You can't add negative money!");
            return false;
        }
        DataBase.getInstance().addMoney(args[0], money);
        String format = DataBase.getInstance().format(money);
        player.sendMessage("You have added " + format + " to " + args[0] + "!");
        return true;
    }
}